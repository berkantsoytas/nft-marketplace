...
contract("NftMarket", (accounts) => {
  ...
  describe("Token transfers", () => {
    ...
    it("should be able to retreive nft by index", async () => {
      ...
      assert.equal(nftId1.toNumber(), 1, "Nft id not correct");
      assert.equal(nftId2.toNumber(), 2, "Nft id not correct");
    });
  ...
  describe("List an Nft", () => {
    before(async () => {
      await _contract.placeNftOnSale(1, _price, {
        from: accounts[1],
        value: _listingPrice,
      });
    });
    ...
    it("should have two listed items", async () => {
      ...
      assert.equal(listedNfts.length, 3, "Invalid length of sales Nfts");
    });
